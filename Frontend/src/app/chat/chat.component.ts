...
export class ChatComponent implements OnInit {
	...
	constructor(
		...
		this.client = io('ws://10.11.1.7:3002?channel_id=' + 1, websocketService.getHeader());
	...
	async openChannel(channelId: number) {
		...
		for (let index = this.msgs$.length; index > 0; index--)
			this.sortMessage(this.msgs$[index - 1]);
		this.client = io('ws://10.11.1.7:3002?channel_id=' + channelId, this.websocketService.getHeader());
		this.setClientEvent();
		this.slideChan();
	}
