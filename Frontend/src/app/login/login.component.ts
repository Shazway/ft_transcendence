...
export class LoginComponent {
	...
	async onClickAuth() {
		let uid;
		await axios.get<string>('http://10.14.3.1:3001/login/getUID')
		.then(function (response) {
			uid = response.data;
		});
		if (!uid)
			return;
		window.location.href = 'https://api.intra.42.fr/oauth/authorize?client_id=' + uid + '&redirect_uri=http%3A%2F%2F10.14.3.1%3A4200%2Fauth&response_type=code';
	}
